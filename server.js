...
async function getWeatherForecast(cidade) {
  try {
    const geoUrl = `https://geocoding-api.open-meteo.com/v1/search?name=${encodeURIComponent(cidade)}&count=1&language=pt&countryCode=BR`;
    const geoResp = await fetch(geoUrl);
    const geoData = await geoResp.json();
    if (!geoData.results || geoData.results.length === 0) {
      return { erro: `Não encontrei a cidade "${cidade}".` };
    }
    ...
    if (!forecastData.daily) {
      return { erro: 'A API de clima não retornou dados de previsão.', detalhe: JSON.stringify(forecastData).slice(0, 300) };
    }

    const dias = forecastData.daily.time.map((data, i) => ({
      data,
      temperatura_maxima: forecastData.daily.temperature_2m_max[i],
      temperatura_minima: forecastData.daily.temperature_2m_min[i],
      chance_de_chuva_percent: forecastData.daily.precipitation_probability_max[i],
      codigo_tempo: forecastData.daily.weather_code[i]
    }));

    return { cidade: name + (admin1 ? ', ' + admin1 : ''), previsao_proximos_dias: dias };
  } catch (err) {
...
- Se o usuário perguntar sobre o tempo, clima ou previsão, use a ferramenta consultar_clima. Se ele não disser a cidade, pergunte qual cidade antes de consultar. O campo "codigo_tempo" retornado é um código meteorológico padrão (WMO); interprete-o em português (ex: céu limpo, nublado, chuva, tempestade) na sua resposta, sem citar o número do código. Se a ferramenta retornar um campo "erro", copie essa mensagem de erro literalmente na resposta (modo de diagnóstico temporário).
