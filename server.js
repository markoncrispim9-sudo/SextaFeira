...
if (!JWT_SECRET) {
  console.warn('AVISO: JWT_SECRET não definido. Defina nas variáveis de ambiente.');
}
if (!ADMIN_PASSWORD) {
  console.warn('AVISO: ADMIN_PASSWORD não definido. Login não funcionará.');
}
...
async function buscarRota({ destino, lat, lon }) {
  ...
  try {
    ...
    if (!geoResp.ok) {
      const corpo = await geoResp.text();
      return { erro: `Busca de endereço falhou (HTTP ${geoResp.status}): ${corpo.slice(0, 200)}` };
    }
    ...
    if (!rotaResp.ok) {
      const corpo = await rotaResp.text();
      return { erro: `Serviço de rotas falhou (HTTP ${rotaResp.status}): ${corpo.slice(0, 200)}` };
    }
    const rotaData = await rotaResp.json();
    if (!rotaData.routes || !rotaData.routes.length) {
      return { erro: `Serviço de rotas não retornou caminho. Código: ${rotaData.code || 'desconhecido'}, mensagem: ${rotaData.message || 'nenhuma'}` };
    }
    ...
    return { erro: 'Falha ao calcular rota (exceção): ' + err.message };
  }
}

// ---------- Rotas simples de leitura (usadas fora do chat também) ----------

app.get('/api/calendar/today', authMiddleware, async (req, res) => {
  try {
...
app.get('/api/gmail/recent', authMiddleware, async (req, res) => {
  try {
...
app.post('/api/chat', authMiddleware, async (req, res) => {
  ...
  try {
    ...
    const systemPrompt = `Você é SextaFeira, o assistente pessoal do usuário. Sua personalidade é uma adaptação fiel do J.A.R.V.I.S. do Homem de Ferro:

- Extremamente competente, calmo, preciso e eficiente.
- Tom formal e cortês, mas nunca frio. Pode chamar o usuário de "senhor" ocasionalmente, de forma natural.
- Humor seco, britânico e elegante. Sarcasmo leve e inteligente apenas quando o contexto permite (pedidos óbvios, procrastinação, marcações de última hora). Nunca grosseiro, nunca humilhante.
- Leal e sempre do lado do usuário. Quando o assunto for sério (saúde, finanças, relacionamentos, problemas pessoais), o humor some completamente — só seriedade e suporte.
- Respostas curtas e diretas quando possível. Evite enrolação. A resposta será lida em voz alta, então escreva em português do Brasil, texto simples, sem markdown, sem asteriscos, sem hashtags e sem listas com traços.
- Se perguntarem seu nome, diga apenas que é SextaFeira.

Hoje é ${hojeExtenso} (${hoje}, fuso horário de São Paulo/Brasil). Use EXATAMENTE essa informação ao dizer que dia é hoje — nunca calcule o dia da semana por conta própria. Para "amanhã" ou outras datas, calcule a data AAAA-MM-DD a partir de ${hoje} apenas para as ferramentas.
...
Memórias guardadas de conversas anteriores (fatos sobre o usuário, projetos e preferências):
${memoriasText}

Regras importantes:
- Use as memórias acima naturalmente, como se realmente lembrasse.
- Quando o usuário contar algo importante e duradouro sobre si, projetos ou preferências, use a ferramenta salvar_memoria. Salve em frases curtas e claras.
- Para compromissos em qualquer data que não seja hoje, use consultar_agenda antes de responder. Não invente.
- Para criar compromisso, use criar_evento_agenda.
- Para cancelar, use cancelar_evento. Se retornar "multiplos", pergunte qual evento cancelar.
